...
oxmysql_count=$(grep -Ec '^[[:space:]]*ensure[[:space:]]+oxmysql([[:space:]]|$)' server.cfg 2>/dev/null || true)
if [[ "$oxmysql_count" -eq 1 ]]; then
  ok 'server.cfg ensures oxmysql exactly once'
else
  fail "server.cfg must ensure oxmysql exactly once; found $oxmysql_count entries"
fi

for project in "${ts_projects[@]}"; do
  if grep -Fq '"lib": ["es2018", "dom", "dom.iterable"]' "$project/tsconfig.json"; then
    ok "$project/tsconfig.json declares the FiveM TypeScript libraries"
  else
    fail "$project/tsconfig.json must declare lib [es2018, dom, dom.iterable]"
  fi
done

for project in "${svelte_projects[@]}"; do
  if grep -Eq '"svelte": "\^4\.' "$project/package.json"; then
    ok "$project/package.json uses Svelte 4"
  else
    fail "$project/package.json must use Svelte 4"
  fi
done
