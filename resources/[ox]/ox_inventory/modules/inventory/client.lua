...
function Inventory.OpenDumpster(entity)
	-- Always use coordinates for dumpsters (fixes non-networked entity issue)
...
local function assertMetadata(metadata, setDefaults)
    ...
    if setDefaults and metadata.rarity == nil then
...
local textPrompts = {
    evidence = {
        ...
        message = ('%s %s'):format("[E]", locale('open_police_evidence'))
    },
    stash = {
        options = { icon = 'fa-warehouse' },
        message = ('%s  %s'):format("[E]", locale('open_stash'))
    }
}
