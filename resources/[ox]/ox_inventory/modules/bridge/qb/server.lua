...
---Auto-stop conflicting resources on resource start
AddEventHandler('onResourceStart', function(resource)
    if resource ~= 'qb-weapons' or resource ~= 'qb-shops' then return end
    StopResource(resource)
end)

---Set compatibility properties for items
---@param item SlotWithItem|nil
---@return SlotWithItem|nil
...
---Setup player inventory and methods
---@param Player table
...
---QBCore initialization and initial cleanup
...
---Run an item's use callback if allowed
---@param source number
...
---Sync player's cash money with inventory cash
AddEventHandler('QBCore:Server:OnMoneyChange', function(src, account, amount, changeType)
    ...
    Inventory.SetItem(src, 'money',
        changeType == "set" and amount
        or changeType == "remove" and item.count - amount
        or changeType == "add" and item.count + amount)
end)

---Returns core player data structure for compatibility
---@param player table
---@return table
...
---Sync inventory and money with player
---@param inv table
function server.syncInventory(inv)
    local accounts = Inventory.GetAccountItemCounts(inv)
    if accounts then
        if not inv.id then inv.id = source end
...
---Buy a license for player (with validation)
---@param inv table
---@param license table
...
---Convert legacy QB inventory data to ox_inventory structure
...
---Checks if player is boss of their job or gang
---@param playerId number
---@return boolean
...
---Checks if player has specified item(s)
---@param source number
...
---Registers a handler for export calls
...
function server.canRobPlayer(targetSource)
	...
	local isCuffed = Player.PlayerData.cuffed or false

	-- Also check state as fallback
	local targetState = Player(targetSource).state

	return isDead or isCuffed or (targetState.canSteal == true)
end
