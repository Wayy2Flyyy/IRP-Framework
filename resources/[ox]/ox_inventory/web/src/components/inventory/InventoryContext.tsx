...
const InventoryContext: React.FC = () => {
  ...
  const handleClick = (data: DataProps) => {
    ...
    switch (data && data.action) {
      case 'use':
        ...
        break;
      case 'give':
        fetchNui('giveItem', { slot: item.slot, count: 1 }); // Default to 1
        break;
      case 'drop':
        isSlotWithItem(item) && onDrop({ item: item, inventory: inventoryType || 'player' });
        break;
      case 'split':
        setInfoVisible(true);
        break;
  ...
  const isArmorItem = (itemName: string | undefined) => {
    return itemName === 'armour' || 
          itemName === 'light_armor' || 
          itemName === 'heavy_armor' || 
          itemName === 'pd_armor';
  };

  const getMaxPlates = (itemName: string | undefined) => {
    switch(itemName) {
      case 'light_armor': return 2;
      case 'armour': return 3;
      case 'heavy_armor': return 4;
      case 'pd_armor': return 5;
      default: return 3;
    }
  };
  ...
  return (
    <>
      ...
      <Menu>
        ...
        {isArmorItem(item?.name) && item && (
          <>
            <MenuItem
              onClick={() => fetchNui('insertplate', { slot: item.slot })}
              label={`Add Plates (${item.metadata?.plates || 0}/${item.metadata?.maxPlates || getMaxPlates(item?.name)})`}
            />
            <MenuItem
              onClick={() => fetchNui('removeplates', { slot: item.slot })}
              label="Remove Plates"
            />
          </>
        )}
