...
CreateThread(function()
    while true do
        Wait(Constants.BACKPACK_CHECK_INTERVAL)

        if PlayerData and PlayerData.loaded then
            local currentTime = GetGameTimer()
            if currentTime - lastBackpackCheck > Constants.BACKPACK_CHECK_INTERVAL then
...
function client.openInventory(inv, data)
    ...
    if inv == 'player' or inv == 'showbpk' and data ~= cache.serverId then
        ...
        if not data then
            ...
            data = GetPlayerServerId(targetId)
        else
...
RegisterNetEvent('ox_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
	...
	for id, data in pairs(lib.load('data.licenses') or {}) do
		local newMsgSK = ('%s %s'):format("[E]", locale('purchase_license', data.name))
		lib.points.new({
			coords = data.coords,
			distance = 16,
			inv = 'license',
...
RegisterNUICallback('swapItems', function(data, cb)
	...
	if data.fromSlot == 2 and isArmorItem(data.item) then
		local ped = PlayerPedId()
		SetPedArmour(ped, 0)
	end

	if data.toSlot == 2 and isArmorItem(data.item) then
...
RegisterNUICallback('craftItem', function(data, cb)
	...
	local success, response = lib.callback.await('ox_inventory:craftItem', 200, id, index, data.fromSlot, data.toSlot, data.count)
...
RegisterNUICallback('setSearching', function(data, cb)
    isSearching = data
    -- Keep input blocked at all times when inventory is open
    -- No need to change SetNuiFocusKeepInput
    cb('ok')
end)